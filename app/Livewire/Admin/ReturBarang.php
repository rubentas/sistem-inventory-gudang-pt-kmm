...
class ReturBarang extends Component
{
  ...
  private function uploadBukti($retur)
  {
    if ($this->bukti_invoice) {
      ...
      $filename = 'RET-PJL-' . now()->format('Ymd') . '-' . $retur->id_retur . '-' . Str::random(6) . '.' . $file->getClientOriginalExtension();
  ...
  public function updateRetur(): void
  {
    ...
    try {
      ...
      foreach ($this->detail as $d) {
        DetailReturPenjualan::where('id_detail_retur', $d['id_detail_retur'] ?? 0)->update([
          'jumlah_retur'   => $d['jumlah_retur'],
          'harga_satuan'   => $d['harga_satuan'],
          'subtotal_retur' => ($d['jumlah_retur'] ?? 0) * ($d['harga_satuan'] ?? 0),
          'alasan'         => $d['alasan'],
  ...
  public function approve(int $id): void
  {
    ...
    if ($nextStatus === 'Cek Kasir' && ! $retur->bukti_invoice) {
      $this->dispatch('dataSaved', type: 'error', title: 'Gagal!', message: 'Bukti invoice wajib diupload sebelum masuk ke Cek Kasir.');
      return;
    }
  ...
  {
    $this->validate(['alasan_tolak' => 'required|string', 'alasan_tolak_lainnya' => 'nullable|string|max:255'], ['alasan_tolak.required' => 'Alasan penolakan wajib dipilih.']);
    $retur                   = ReturPenjualan::findOrFail($this->id_retur);
    $alasanFinal             = $this->alasan_tolak === 'Lainnya' && $this->alasan_tolak_lainnya ? $this->alasan_tolak_lainnya : $this->alasan_tolak;
  ...
  {
    $this->reset(['id_retur', 'id_order', 'detail', 'nama_toko', 'id_sales', 'isEdit', 'editMode', 'tolakMode', 'alasan_tolak', 'alasan_tolak_lainnya', 'bukti_invoice']);
    $this->tanggal_retur = now()->format('Y-m-d');
  ...
  {
    $this->orderList = OrderSales::whereIn('status', ['selesai', 'diproses'])->with('barang')->orderByDesc('tanggal_order')->limit(200)->get()->toArray();
  ...
  {
    return redirect()->route('admin.retur-barang.pdf', ['search' => $this->search, 'filterStatus' => $this->filterStatus]);
  }
}
