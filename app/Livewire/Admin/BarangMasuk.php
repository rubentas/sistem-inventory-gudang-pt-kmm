...
class BarangMasuk extends Component {
  ...
  public $bukti_pembayaran; // Upload bukti
  ...
  protected $messages = [
    ...
    'bukti_pembayaran.max'   => 'File maksimal 2MB.',
  ];

  public function mount(): void {$this->tanggal_masuk = now()->format('Y-m-d');}
  ...
  public function setFilter(string $type): void {
    ...
    switch ($type) {
    case 'today':$this->filterDate = now()->format('Y-m-d');
      break;
    case 'week':$this->filterDate = now()->format('Y-m-d');
      break;
    case 'month':$this->filterDate = now()->startOfMonth()->format('Y-m-d');
      break;
    default: $this->filterType = 'custom';
      break;
    }
    $this->resetPage();
  }
  ...
  public function resetFilters(): void {
    $this->search = $this->filterDate = $this->filterSupplier = $this->filterType = '';
    $this->resetPage();
  }

  public function resetForm(): void {
    $this->reset([
      'id_barang', 'id_supplier', 'no_nota', 'no_surat_jalan',
      'jumlah', 'sumber', 'keterangan', 'editId', 'isEdit', 'satuan_display', 'bukti_pembayaran',
    ]);
  ...
  public function updatedIdBarang($value): void {
    $barang               = $value ? Barang::find($value) : null;
    $this->satuan_display = $barang->satuan ?? 'Pcs';
  }

  public function updatedSumber($value): void {if ($value !== 'Supplier') {
    $this->id_supplier = '';
  }}
  ...
  public function simpan(): void {
    ...
    if ($this->bukti_pembayaran) {
      $data['bukti_pembayaran'] = $this->bukti_pembayaran->store('dokumen/bukti-masuk', 'public');
    }

    $item = BarangMasukModel::create($data);

    // Auto generate invoice
    $noInvoice = 'INV-MSK/' . date('Ymd') . '/' . str_pad($item->id_masuk, 5, '0', STR_PAD_LEFT);
    $item->update(['no_invoice' => $noInvoice]);

    $this->syncStok($this->id_barang, (int) $this->jumlah, 'increment');

    $this->resetForm();
    $this->dispatch('dataSaved', type: 'success', title: 'Berhasil!', message: 'Barang masuk & invoice berhasil disimpan.');
  }
  ...
  private function syncStok(int $idBarang, int $jumlah, string $action = 'increment'): void {
    ...
    if ($stok) {
      $action === 'increment' ? $stok->increment('jumlah_stok', $jumlah) : $stok->decrement('jumlah_stok', $jumlah);
      $stok->updated_at = now();
      $stok->save();
    } elseif ($action === 'increment') {
      $barang = Barang::find($idBarang);
      Stok::create(['id_barang' => $idBarang, 'jumlah_stok' => $jumlah, 'stok_minimum' => $barang?->stok_minimum ?? 0, 'updated_at' => now()]);
    }
  }
  ...
  public function getStats(): array {
    return [
      ...
      'thisMonth'  => BarangMasukModel::whereMonth('tanggal_masuk', now()->month)->whereYear('tanggal_masuk', now()->year)->count(),
    ];
  }
    ...
    $barangMasuk = BarangMasukModel::with(['barang', 'supplier', 'user'])
      ->when($this->search, function ($query) {
        $query->where(function ($sub) {
          $sub->whereHas('barang', fn($q) => $q->where('nama_barang', 'like', '%' . $this->search . '%')->orWhere('kode_barang', 'like', '%' . $this->search . '%'))
            ->orWhereHas('supplier', fn($q) => $q->where('nama_supplier', 'like', '%' . $this->search . '%'))
      ...
      ->when($this->filterDate, function ($query) {
        ...
        case 'week':return $query->whereBetween('tanggal_masuk', [now()->subDays(6)->startOfDay(), now()->endOfDay()]);
        case 'month':return $query->whereBetween('tanggal_masuk', [$date->copy()->startOfMonth(), $date->copy()->endOfMonth()]);
        default: return $query->whereDate('tanggal_masuk', $this->filterDate);
        }
      })
      ->when($this->filterSupplier, fn($q) => $q->where('id_supplier', $this->filterSupplier))
      ->orderByDesc('tanggal_masuk')->orderByDesc('id_masuk')->paginate(10);
