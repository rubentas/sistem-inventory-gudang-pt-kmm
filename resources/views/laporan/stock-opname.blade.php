  ...
  <table>
    ...
    <tbody>
      @forelse($data as $index => $item)
        <tr>
          <td>{{ $index + 1 }}</td>
          <td class="text-left">{{ $item->tanggal_opname->translatedFormat('d/m/Y') }}</td>
          <td class="text-left">{{ $item->barang->nama_barang ?? '-' }}</td>
          <td class="text-right">{{ number_format($item->stok_sistem) }}</td>
          <td class="text-right">{{ number_format($item->stok_fisik) }}</td>
          <td class="text-right text-bold {{ $item->selisih != 0 ? 'style=color:#dc2626' : '' }}">
    ...
    <tfoot>
      <tr style="background: #111827; color: white;">
        <td colspan="5" class="text-left text-bold">TOTAL SELISIH</td>
        <td class="text-right text-bold">{{ number_format($total_selisih) }}</td>
        <td colspan="2"></td>
      </tr>
    </tfoot>
  </table>
