<div>
  <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    <div>
      ...
      <p class="text-gray-500 text-sm">Mengelola pesanan dari para sales</p>
    </div>
  ...
  @if ($showForm)
    <div class="bg-white rounded-lg shadow p-5 mb-6">
      <h2 class="text-lg font-semibold text-gray-800 mb-4">{{ $isEdit ? 'Edit' : 'Tambah' }} Order Sales</h2>
        ...
        <tbody class="divide-y divide-gray-200">
          @forelse($orders as $order)
            <tr class="hover:bg-gray-50">
              ...
              <td class="px-4 py-3 text-sm text-gray-600">{{ number_format($order->jumlah) }}</td>
              ...
              <td class="px-4 py-3 text-sm">
                <button wire:click="bukaEdit({{ $order->id_order }})"
          ...
          @empty
            <tr>
              <td colspan="7" class="px-4 py-8 text-center text-gray-500">Tidak ada data order sales</td>
            </tr>
          @endforelse
        </tbody>
      </table>
