    ...
    <div class="px-6 py-5 sm:px-8 sm:py-6">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-5">
        ...
        <div class="flex flex-wrap items-center gap-3">
          <a href="{{ route('admin.retur-pembelian.pdf', ['search' => $search, 'filterSupplier' => $filterSupplier]) }}" target="_blank"
      ...
      <table class="w-full min-w-[800px]">
        <thead>
          <tr class="bg-gray-50 border-b border-gray-100">
            ...
            <th class="px-5 py-4 text-left text-xs font-bold text-gray-400 uppercase tracking-wider">No Retur</th>
            <th class="px-5 py-4 text-left text-xs font-bold text-gray-400 uppercase tracking-wider">Supplier</th>
            <th class="px-5 py-4 text-left text-xs font-bold text-gray-400 uppercase tracking-wider">Barang</th>
            <th class="px-5 py-4 text-right text-xs font-bold text-gray-400 uppercase tracking-wider">Jumlah</th>
            <th class="px-5 py-4 text-left text-xs font-bold text-gray-400 uppercase tracking-wider">Tujuan</th>
            <th class="px-5 py-4 text-left text-xs font-bold text-gray-400 uppercase tracking-wider">Tanggal</th>
          </tr>
        </thead>
        <tbody class="divide-y divide-gray-50">
          @forelse ($returs as $index => $r)
            <tr class="hover:bg-blue-50/30 transition">
              <td class="px-5 py-4 text-xs font-semibold text-gray-300">{{ $returs->firstItem() + $index }}</td>
              <td class="px-5 py-4 text-sm font-mono font-semibold text-blue-600">{{ $r->no_retur }}</td>
              <td class="px-5 py-4 text-sm text-gray-800">{{ $r->supplier->nama_supplier ?? '-' }}</td>
              <td class="px-5 py-4 text-sm font-bold text-gray-900">{{ $r->barang->nama_barang ?? '-' }}</td>
              <td class="px-5 py-4 text-sm text-right font-bold text-gray-700">{{ $r->jumlah }}</td>
              <td class="px-5 py-4 text-sm text-gray-600">{{ $r->tujuan }}</td>
              <td class="px-5 py-4 text-sm text-gray-600">{{ $r->tanggal_retur->format('d/m/Y') }}</td>
            </tr>
          @empty
            <tr>
