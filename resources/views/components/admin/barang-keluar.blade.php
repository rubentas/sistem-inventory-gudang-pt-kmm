    ...
    <div class="px-6 py-5 sm:px-8 sm:py-6">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-5">
        ...
        <div class="flex flex-wrap items-center gap-3">
          ...
          <a href="{{ route('laporan.keluar.pdf', ['tanggal_awal' => $filterDate ?? now()->startOfMonth()->format('Y-m-d'), 'tanggal_akhir' => $filterDate ?? now()->format('Y-m-d')]) }}"
            target="_blank"
      ...
      <div class="flex flex-col sm:flex-row gap-2.5">
        ...
        <div class="relative shrink-0" x-data="{ showFilter: false }" @click.outside="showFilter = false">
          ...
          <div x-show="showFilter" x-cloak x-transition
            ...
            <p class="text-sm font-black text-gray-900 mb-4">Filter Data</p>
            <div class="space-y-4">
              <div>
        ...
        <tbody class="divide-y divide-gray-50">
          @forelse($barangKeluar as $index => $item)
            <tr class="hover:bg-blue-50/30 transition">
              ...
              <td class="px-5 py-4">
                <span class="text-base font-bold text-orange-600">{{ number_format($item->jumlah) }}</span>
                <span class="text-xs text-gray-400 ml-1">unit</span>
              </td>
              <td class="px-5 py-4 text-sm font-medium text-gray-700">{{ $item->wilayah->nama_wilayah ?? '—' }}</td>
              <td class="px-5 py-4 text-sm text-gray-500 font-mono">#{{ $item->id_order ?? '—' }}</td>
              <td class="px-5 py-4">
          ...
          <div>
            ...
            <select wire:model.live="id_order"
              ...
              @foreach ($orders as $order)
                <option value="{{ $order->id_order }}">#{{ $order->id_order }} —
                  {{ $order->barang->nama_barang ?? '' }} ({{ $order->jumlah }} unit) —
                  {{ $order->wilayah->nama_wilayah ?? '' }}</option>
              @endforeach
            </select>
            @error('id_order')
