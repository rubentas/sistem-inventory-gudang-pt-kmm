<div>
  <div class="mb-6">
    <h1 class="text-2xl font-bold text-gray-800">Dashboard Sales</h1>
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
    <div class="bg-white rounded-lg shadow p-5 border-l-4 border-orange-500">
      ...
      <p class="text-xs text-gray-400 mt-1">Total order hari ini</p>
    </div>

    <div class="bg-white rounded-lg shadow p-5 border-l-4 border-yellow-500">
      <p class="text-gray-500 text-sm">Pending</p>
      <p class="text-2xl font-bold text-yellow-600">{{ number_format($totalPending) }}</p>
      <p class="text-xs text-gray-400 mt-1">Menunggu diproses</p>
    </div>

    <div class="bg-white rounded-lg shadow p-5 border-l-4 border-blue-500">
      <p class="text-gray-500 text-sm">Diproses</p>
      <p class="text-2xl font-bold text-blue-600">{{ number_format($totalDiproses) }}</p>
      <p class="text-xs text-gray-400 mt-1">Sedang diproses admin</p>
    </div>

    <div class="bg-white rounded-lg shadow p-5 border-l-4 border-green-500">
      <p class="text-gray-500 text-sm">Selesai</p>
      <p class="text-2xl font-bold text-green-600">{{ number_format($totalSelesai) }}</p>
      <p class="text-xs text-gray-400 mt-1">Sudah dikirim</p>
    </div>
  </div>
      ...
      <table class="min-w-full">
        <thead class="bg-gray-50">
          <tr>
            ...
            <th class="px-5 py-3 text-left text-xs font-medium text-gray-500 uppercase">Wilayah</th>
        ...
        <tbody class="divide-y divide-gray-200">
          @forelse($orderTerbaru as $order)
            <tr class="hover:bg-gray-50">
              ...
              <td class="px-5 py-3 text-sm text-gray-600">{{ $order->wilayah->nama_wilayah ?? '-' }}</td>
              <td class="px-5 py-3 text-sm text-gray-600">{{ number_format($order->jumlah) }}
          ...
          @empty
            <tr>
              <td colspan="5" class="px-5 py-8 text-center text-gray-500">Belum ada order</td>
            </tr>
          @endforelse
        </tbody>
      </table>
